...
class _DataSplitter():
    ...
    def store_test_data(self):
        ...
        if len(os.listdir(self.test_dir['enzymes'])) > 0:
            log.warning("CAUTION: This method is meant to be executed only once. "
                        ...
                        "the size of the train set as merging is not implemented yet. "
    ...
    def _split_trainval(self):
        ...
        for cls in leaf_classes:
            ...
            with open(path_to_cls, 'r') as f:
                prot_codes_in_cls = [pc.strip() for pc in f.readlines()]
                val_indices = np.random.choice(np.arange(len(prot_codes_in_cls)),
                                               replace=False,
                                               size=int(len(prot_codes_in_cls)
        ...
        import cPickle
        with open(os.path.join(self.trainval_dir['enzymes_proc'], 'train_data.pickle'), 'rb') as f:
            cPickle.dump(train_enzymes, f)
        with open(os.path.join(self.trainval_dir['enzymes_proc'], 'val_data.pickle'), 'rb') as f:
