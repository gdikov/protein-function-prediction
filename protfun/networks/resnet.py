...
def resnet(input, n_outputs, last_nonlinearity):
    """
    resnet is a deep network with residual connections in a fashion similar to dense_net.
    The difference here is that all the connections are summed up instead of concatenated before
    they are fed into the next layer.
    ...
    Usage::
        ...
        >>> n_classes = 2
        >>> # apply the network
        >>> output_layer, l2_terms = resnet(inputs, n_classes, sigmoid)
