...
class ProgressView(object):
    ...
    def save(self):
        ...
        self._save(artifacts=['train_accuracy', 'val_accuracy'], y_range=[0, 2],
        ...
        for artifact in artifacts:
            ...
            if values.size != 0:
                ...
                for i in range(values.shape[1]):
                    val_i = values[:, i]
                    if artifact == "train_loss" or artifact == "train_accuracy":
                        val_i = self.running_mean(values[:, i], self.mean_window)
                    ax.plot(val_i, label='{} {}'.format(artifact, i))
            else:
